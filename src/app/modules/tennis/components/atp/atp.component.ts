  ...
  convertZeroesToTruthy() {
    for (var i = 0; i < this.tournamentInfo.data.matches.length; i++) {
      if(this.tournamentInfo.data.matches[i].player_one.line_score) {
        for (var j = 0; j < this.tournamentInfo.data.matches[i].player_one.line_score.length; j++) {
          this.tournamentInfo.data.matches[i].player_one.line_score[j] = this.tournamentInfo.data.matches[i].player_one.line_score[j].toString();
        }
        for (var j = 0; j < this.tournamentInfo.data.matches[i].player_two.line_score.length; j++) {
          this.tournamentInfo.data.matches[i].player_two.line_score[j] = this.tournamentInfo.data.matches[i].player_two.line_score[j].toString();
        }
      }
    }
  }
